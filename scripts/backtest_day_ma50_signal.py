"""
简化版信号回放回测：日线收盘在 MA50 之上 + 30M 任意买点。
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="信号回放回测：日线收盘在 MA50 之上 + 30M 任意买点",
...
def main() -> None:
    ...
    print(
        f"规则: 日线收盘在 MA{args.ma_period} 之上 + 30M任意买点({', '.join(args.buy_types)})，N={args.horizon}日信号评估，入场={args.entry_mode}，止损={args.stop_loss_pct}%"
    )
    ...
    csv_path = out_dir / f"day_ma{args.ma_period}_30m_signal_rows_{ts}.csv"
    json_path = out_dir / f"day_ma{args.ma_period}_30m_signal_summary_{ts}.json"
