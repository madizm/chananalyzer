...
# 配置日志：默认保持安静，避免在正常搜索结果中混入日志/底层错误信息；
# 需要排障时通过 --debug 打开详细日志。
logging.basicConfig(
...
def _load_dotenv() -> None:
    ...
    IWENCAI_DOTENV_PATH 指定文件。已存在的系统环境变量不会被覆盖。
    """
    explicit_path = os.getenv("IWENCAI_DOTENV_PATH")
    candidates = []
    ...
    if explicit_path:
        ...
        for base in (Path.cwd(), Path(__file__).resolve()):
            for parent in (base if base.is_dir() else base.parent, *base.parents):
                env_path = parent / ".env"
    ...
    for env_path in candidates:
        ...
        try:
            ...
            logger.debug("已加载.env配置: %s", env_path)
        except Exception as e:
            logger.debug("加载.env失败: %s - %s", env_path, e)
...
class NewsSearchAPI:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        """
        self.base_url = os.getenv("IWENCAI_BASE_URL", "https://openapi.iwencai.com").rstrip("/")
...
def _print_api_error(response: Dict[str, Any], debug: bool = False, context: str = "") -> None:
    """CLI友好错误输出：默认不展示底层响应，debug时才完整输出。"""
    prefix = f"{context} " if context else ""
    if debug:
        print(f"\n{prefix}API错误响应（调试信息）:", file=sys.stderr)
        print(json.dumps(response, indent=2, ensure_ascii=False), file=sys.stderr)
    else:
        print(f"{prefix}新闻搜索暂时不可用，已跳过该查询；如需排障请使用 --debug。", file=sys.stderr)


def _print_exception(message: str, exc: Exception, debug: bool = False) -> None:
    """CLI友好异常输出：避免在正常模式泄露堆栈、网关响应或本地路径。"""
    if debug:
        logger.error(f"{message}: {str(exc)}")
    else:
        print(f"{message}；如需排障请使用 --debug。", file=sys.stderr)
...
数据来源: 同花顺问财财经资讯搜索
    ...
    try:
        ...
        if args.input:
            ...
            for query in queries:
                ...
                for sub_query in sub_queries:
                    try:
                        ...
                        # 检查是否为错误响应：默认只给友好提示，避免混入底层错误信息
                        if "error" in response:
                            _print_api_error(response, args.debug, f"查询 '{query}' - 子查询 '{sub_query}'")
                            continue
                        ...
                        _print_exception(f"查询 '{sub_query}' 失败", e, args.debug)
            ...
            for sub_query in sub_queries:
                try:
                    ...
                    # 检查是否为错误响应：默认只给友好提示，避免混入底层错误信息
                    if "error" in response:
                        _print_api_error(response, args.debug)
                        continue
                    ...
                    _print_exception(f"子查询 '{sub_query}' 失败", e, args.debug)
        ...
        _print_exception("配置或参数错误", e, args.debug)
        sys.exit(1)
    except Exception as e:
        _print_exception("新闻搜索执行失败", e, args.debug)
        if args.debug:
            import traceback
            traceback.print_exc()
        sys.exit(1)
