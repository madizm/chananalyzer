...
- 优先使用本技能自带脚本调用，避免临时拼接复杂 `curl` 命令导致 shell 引号/变量展开错误：
  ```bash
  python3 .pi/skills/news-search/scripts/news_search.py -q "苏大维格 新闻" -l 5 -d 3650
  ```
- 正常回答用户时，不展示堆栈、网关错误体、本地路径、命令行错误或 API Key；只给出简洁结论和必要提示。
- 只有用户明确要求排障时，才使用 `--debug` 并引用调试信息。
- 如需直接调用接口，读取 `IWENCAI_BASE_URL` 和 `IWENCAI_API_KEY` 环境变量，不要硬编码密钥。
