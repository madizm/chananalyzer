...
DEFAULT_OUTPUT_BASE = ROOT_DIR / "Debug" / "model_output" / "strategy_demo8_scan" / "skill_runs"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Analyze specified stocks with Chan demo8 BSP probability scanner.")
...
def build_markdown(
    ...
    lines.extend([
        "### 候选信号",
        "",
        "| 股票 | 方向 | 时间 | 价格 | 概率 | 笔idx | 关键解读 |",
        "| --- | --- | --- | ---: | ---: | ---: | --- |",
    ])
    if rows_for_table:
        for row in rows_for_table:
            lines.append(
                "| {code} | {side} | {time} | {price} | {prob} | {bi} | {interp} |".format(
    ...
    if rows_for_table:
        ...
        lines.extend([
            "### 最高分信号关键特征",
            "",
            f"- 股票/方向：{stock_display(top.get('code', '-'), stock_info)} / {signal_label(top.get('signal_side', '-'))}",
...
def main() -> int:
    ...
    cmd = [
        sys.executable,
        str(ROOT_DIR / "Debug" / "scan_demo8_bsp_probability.py"),
        "--codes",
        scanner_codes,
        "--begin-time",
        args.begin_time,
