...
使用示例:
  ...
  python research_report_search.py -h
            ...
            if file_format == "txt":
                ...
                with open(filepath, 'r', encoding='utf-8-sig', newline='') as f:
                    ...
                    if reader.fieldnames:
                        query_field = 'query' if 'query' in reader.fieldnames else reader.fieldnames[0]
                        queries = [row.get(query_field, '').strip() for row in reader if row.get(query_field, '').strip()]
                    else:
                        f.seek(0)
                        raw_reader = csv.reader(f)
                        queries = [row[0].strip() for row in raw_reader if row and row[0].strip()]
    ...
    def _dedupe_articles(self, articles: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """按研报唯一标识/链接/标题日期去重，保留排序后的首条。"""
        deduped = []
        seen = set()
        for article in articles:
            key = article.get("uid") or article.get("url") or (
