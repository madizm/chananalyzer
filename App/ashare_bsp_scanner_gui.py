...
数据来源:
    - 使用本地缓存数据库 chan.db 获取K线数据（从tushare缓存）

过滤规则:
    - 扫描数据库中所有有数据的股票（包含科创板）
    - 剔除北交所（8xx, 43x）、B股（200, 900）、CDR（920）
    - 剔除停牌超过15天的股票
...
def get_tradable_stocks():
    """
    从本地缓存数据库获取所有可交易的A股股票列表

    过滤条件:
        1. 剔除北交所（8开头、43开头）
        2. 剔除B股（200开头深圳B股、900开头上海B股）
        3. 剔除存托凭证CDR（920开头）
        注意：科创板(688)保留，ST股票需要名称信息暂不过滤
    ...
    """
    # 获取数据库路径
    db_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "chan.db")
    ...
    try:
        ...
        # 获取所有股票代码及其最新K线数据
        query = """
            SELECT code, MAX(date) as latest_date, close
            FROM kline_data
            WHERE kl_type = 'DAY'
            GROUP BY code
            ORDER BY code
        ...
        stock_list = []
        for code, latest_date, close in rows:
            # 过滤条件
            # 1. 剔除北交所（8开头、43开头）
            ...
            if code.startswith('920'):
                continue
            # 注意：ST股票过滤需要股票名称，数据库中暂无存储，暂不过滤
            # 如需过滤ST股票，可手动添加ST股票代码列表或从外部接口获取

            stock_list.append({
                '代码': code,
                '名称': code,  # 数据库中没有名称，使用代码代替
                '最新价': float(close) if close else 0,
                '涨跌幅': 0.0,  # 需要计算，这里暂时设为0
            })
