...
KEY_FEATURES = (*BASE_KEY_FEATURES, *SECOND_KEY_FEATURES)
        ...
        for signal_side in signal_sides:
            ...
            for bi in level_chan.bi_list:
                ...
                row = {
                    ...
                    "target_bsp_types": ",".join(sorted(TARGET_BSP_TYPES)),
...
def init_scan_db(conn: sqlite3.Connection) -> None:
    conn.executescript(
        """
        CREATE TABLE IF NOT EXISTS demo9_bsp_probability_scan_runs (
        ...
        CREATE TABLE IF NOT EXISTS demo9_bsp_probability_scan_signals (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            ...
            prev_first_bsp_exists REAL,
            prev_first_bsp_type_1 REAL,
            prev_first_bsp_type_1p REAL,
            prev_first_bsp_bi_gap REAL,
            prev_first_bsp_klu_gap REAL,
            entry_vs_prev_first_bsp_price REAL,
            retracement_from_prev_first REAL,
            prev_first_bsp_divergence_rate REAL,
            model_dir TEXT NOT NULL,
            created_at TEXT NOT NULL,
            FOREIGN KEY(run_id) REFERENCES demo9_bsp_probability_scan_runs(id)
        );

        CREATE INDEX IF NOT EXISTS idx_demo9_bsp_prob_runs_created_at
            ON demo9_bsp_probability_scan_runs(created_at);
        CREATE INDEX IF NOT EXISTS idx_demo9_bsp_prob_signals_run_id
            ON demo9_bsp_probability_scan_signals(run_id);
        CREATE INDEX IF NOT EXISTS idx_demo9_bsp_prob_signals_code
            ON demo9_bsp_probability_scan_signals(code);
        CREATE INDEX IF NOT EXISTS idx_demo9_bsp_prob_signals_open_time
            ON demo9_bsp_probability_scan_signals(open_time);
        CREATE INDEX IF NOT EXISTS idx_demo9_bsp_prob_signals_probability
            ON demo9_bsp_probability_scan_signals(probability);
        """
    )
    conn.commit()
...
def save_scan_to_db(
    *,
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        cursor.execute(
            """
            INSERT INTO demo9_bsp_probability_scan_runs (
                started_at, finished_at, begin_time, end_time,
                model_kl_type, parent_kl_type, child_kl_type,
                target_group, target_bsp_types, dependency_bsp_types, main_bs_type,
        ...
        cursor.execute(
            "UPDATE demo9_bsp_probability_scan_runs SET summary_json = ? WHERE id = ?",
        ...
        for row in rows:
            ...
            signal_rows.append((
                ...
                optional_float(row.get("prev_first_bsp_exists")),
                optional_float(row.get("prev_first_bsp_type_1")),
                optional_float(row.get("prev_first_bsp_type_1p")),
                optional_float(row.get("prev_first_bsp_bi_gap")),
                optional_float(row.get("prev_first_bsp_klu_gap")),
                optional_float(row.get("entry_vs_prev_first_bsp_price")),
                optional_float(row.get("retracement_from_prev_first")),
                optional_float(row.get("prev_first_bsp_divergence_rate")),
                row["model_dir"],
                created_at,
            ))
        cursor.executemany(
            """
            INSERT INTO demo9_bsp_probability_scan_signals (
                run_id, code, signal_side, target_group, target_bsp_types,
                open_time, bi_idx, klu_idx, price, probability, hit_min_prob, threshold_hits,
                candidate_divergence_rate, candidate_break_prev_extreme,
                entry_close_pos, child_close_pos, parent_range, ma_dist_10,
                prev_bsp_divergence_rate,
                prev_first_bsp_exists, prev_first_bsp_type_1, prev_first_bsp_type_1p,
                prev_first_bsp_bi_gap, prev_first_bsp_klu_gap,
                entry_vs_prev_first_bsp_price, retracement_from_prev_first,
                prev_first_bsp_divergence_rate, model_dir, created_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            signal_rows,
        )
...
def parse_args():
    parser = argparse.ArgumentParser(description="使用 demo9 训练模型扫描30M确认笔的二类买卖点概率。")
...
if __name__ == "__main__":
    ...
    print(f"扫描股票数量: {len(codes)}, target_group={LABEL_GROUP}, signal_side={args.signal_side}, recent={recent_desc}, workers={worker_count}")
